...
def nuc_count(inp_dict, nuc):
    """Uses the output generated by the pos_checker function, more specifically support for each variant-type,
    as well as the support for the reference and variant call for str1 and str2. to return a dict with dicts giving the
    support for the nucleotide for str1, str2 and each variant type
